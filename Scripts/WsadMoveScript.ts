...
export function WsadScript(scene: SceneAbstract) {
    ...
    scene.camera.lookAt.set(1, scene.camera.lookAt.get(1) + dy * dforward + ddy * dleft + dup);    
    ...
    scene.camera.cam.set(1, scene.camera.cam.get(1) + dy * dforward + ddy * dleft + dup);
